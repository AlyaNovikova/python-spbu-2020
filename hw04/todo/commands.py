...
class BaseDoneCommand(object):
    def perform(self, store, action_name):
        print('Select item:')
        ...
        while True:
            try:
                selected_task = self._select_task(store)
            except ValueError:
                print('Bad input, try again.')
            except IndexError:
                print('Wrong index, try again.')
        ...
        new_status = True if action_name == 'Done' else False

        selected_task.done = new_status
        print('{0}: {1}'.format(action_name, selected_task))
        print()

    def _select_task(self, store):
        selection = int(input('Input number: '))
        if selection < 0:
            raise IndexError('Index needs to be >0')
...
class DoneCommand(BaseCommand, BaseDoneCommand):
    ...
    def perform(self, store):
        BaseDoneCommand.perform(self, store, 'Done')
...
class UndoneCommand(BaseCommand, BaseDoneCommand):
    ...
    def perform(self, store):
        BaseDoneCommand.perform(self, store, 'Undone')
